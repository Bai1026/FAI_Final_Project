...
class MCCFR:
    ...
    def train(self, initial_hands, initial_round_state):
        for i in range(self.iterations):
            for player_pos in [0, 1]:  # 每次迭代时，对每个位置进行训练
                state = self.init_game_state(initial_hands, initial_round_state, player_pos)
                self.mccfr(state, player=player_pos, pi=1.0)
    ...
    def init_game_state(self, initial_hands, initial_round_state, player_pos):
        player_hands = initial_hands.copy()
        if player_pos == 1:
            player_hands.reverse()  # 反轉手牌，模擬不同位置
        return {
            'player_hands': player_hands,
            'community_cards': initial_round_state['community_card'],
            'current_bet': 0,
            'current_player': player_pos,
            'player_chips': [player['stack'] for player in initial_round_state['seats']],
            'pot': 0,
...
class MCCFRPlayer(BasePokerPlayer):
    ...
    def train_model(self):
        ...
        self.mccfr.train(initial_hands, initial_round_state)
        with open('regret_tracking.json', 'w') as f:
            json.dump(self.regret_tracking, f)
            print('Output regret_tracking.json')
